...
mod integration {
    ...
    fn test_is_virtual_smoke() {
        ...
        assert!(result.is_ok(), "is_virtual returned an error: {:?}", result);
    ...
    fn test_hydrate_dehydrate_roundtrip() {
        ...
        assert!(
            rt.block_on(vfs.is_virtual(&path))
                .expect("is_virtual after dehydrate"),
        ...
        assert!(
            !rt.block_on(vfs.is_virtual(&path))
                .expect("is_virtual after hydrate"),
